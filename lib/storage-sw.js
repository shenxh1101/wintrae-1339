export const StorageSW = {
  KEYS: {
    ...
    IMPORT_EXPORT_HISTORY: 'importExportHistory'
  },
  ...
  async getImportExportHistory() {
    return await this.get(this.KEYS.IMPORT_EXPORT_HISTORY, []);
  },

  async addImportExportHistory(entry) {
    const history = await this.getImportExportHistory();
    entry.id = this.generateId();
    entry.timestamp = Date.now();
    history.unshift(entry);
    const trimmed = history.slice(0, 50);
    await this.set(this.KEYS.IMPORT_EXPORT_HISTORY, trimmed);
    return entry;
  },

  async clearImportExportHistory() {
    return await this.set(this.KEYS.IMPORT_EXPORT_HISTORY, []);
  }
};
